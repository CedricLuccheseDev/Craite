...
const includedSamples = computed(() =>
  knownCategories.value
    .filter(c => !configStore.excludedCategories.has(c.name))
        ...
        <div v-for="group in groupedTree" :key="group.key" class="group-block">
          ...
          <button class="group-header" @click="toggleGroup(group.categories)">
            <div class="flex items-center gap-2 flex-1 min-w-0">
              ...
              >
                <UIcon
                  v-if="groupState(group.categories) === 'all'"
                  name="i-lucide-check"
                  class="text-[10px]"
                />
                <span v-else-if="groupState(group.categories) === 'partial'" class="w-2 h-0.5 bg-current rounded-full" />
              </span>
